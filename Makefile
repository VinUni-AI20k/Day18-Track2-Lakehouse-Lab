...
spark-smoke: ## [spark] Smoke test inside Spark container
	$(COMPOSE) exec -T spark env PYTHONPATH=/usr/local/spark/python:/usr/local/spark/python/lib/py4j-0.10.9.7-src.zip:/workspace/scripts /opt/conda/bin/python /workspace/scripts/verify.py

spark-data: ## [spark] Generate 1M-row Bronze (Spark version)
	$(COMPOSE) exec -T spark env PYTHONPATH=/usr/local/spark/python:/usr/local/spark/python/lib/py4j-0.10.9.7-src.zip:/workspace/scripts /opt/conda/bin/python /workspace/scripts/generate_data.py
