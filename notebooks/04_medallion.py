...
# %%
checks = {
    "Bronze/Silver/Gold exist on storage": all(
        Path(layer).exists() for layer in (BRONZE, SILVER, GOLD)
    ),
    "Silver dedup reduced row count": silver_n < bronze_n,
    "Gold covers ≥ 7 dates × 3 models": n_dates >= 7 and n_models >= 3,
    "Gold cost and error metrics populated": (
