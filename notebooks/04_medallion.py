...
checks = {
    ...
    "Gold has date x model rows":     gold_df.height == n_dates * n_models,
    "Gold metrics contain no nulls":  all(gold_df[c].null_count() == 0 for c in metric_columns),
    "p95 is never below p50":         bool((gold_df["p95_latency_ms"] >= gold_df["p50_latency_ms"]).all()),
    "cost and error_rate are nonzero": gold_df["cost_usd"].min() > 0 and gold_df["error_rate"].min() > 0,
}
